...
const FitMapToPoints = ({ points }: { points: [number, number][] }) => {
  ...
  useEffect(() => {
    if (points.length === 0) { map.setView(HOME_FALLBACK_CENTER, 13); return; }
...
const Dashboard = () => {
          ...
          .map((inc: any, idx: number) => {
            ...
            return {
              ...
              timestamp: ts && !Number.isNaN(ts.getTime())
                ? ts.toLocaleString("en-US", { month: "short", day: "2-digit", hour: "2-digit", minute: "2-digit" })
                : "Just now",
              severity:
  ...
  return (
    <div className="p-8 space-y-8">
      ...
      {/* ✅ Welcome Section — forced large size via inline style */}
      <div className="flex justify-between items-start pt-4">
        <div>
          <h1 style={{ fontSize: "3rem", fontWeight: 800, lineHeight: 1.15, color: "#0f172a", letterSpacing: "-0.02em", marginBottom: "0.5rem" }}>
            Welcome back, {userName}
          </h1>
          <p style={{ fontSize: "1.05rem", color: "#64748b" }}>
            System Overview — Here's what's happening with your irrigation system today
          </p>
        </div>
      </div>
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            <div className="rounded-xl border overflow-hidden">
              <MapContainer center={HOME_FALLBACK_CENTER} zoom={13} style={{ height: "460px", width: "100%" }}>
                <TileLayer
